...
class TypeConverter {
    ...
    @TypeConverter
    fun toListInt(rawString: String): List<Int> = rawString.split(DELIMITER).map(String::toInt)

    @TypeConverter
    fun fromListInt(intList: List<Int>): String = intList.joinToString(DELIMITER)

    @TypeConverter
    fun toListString(rawString: String): List<String> = rawString.split(DELIMITER)
